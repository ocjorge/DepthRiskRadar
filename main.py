...
class RiskRadarSystem:
    ...
    def _run_full_detection(self, frame):
        ...
        cal_y = int(self.proc_height * self.config['CALIBRATION_POINT_Y_FACTOR'])
        cal_x = self.proc_width // 2
        cal_region = relative_depth_map[cal_y - 2:cal_y + 2, cal_x - 2:cal_x + 2]
        ...
        self.depth_scale_factor = self.config[
                                      'CALIBRATION_DISTANCE_METERS'] / value_at_cal_point if value_at_cal_point > 1e-6 else 1.0
    ...
    def _update_risk_and_visualize(self, frame, detections, frame_number, timestamp):
        ...
        if self.last_distance_map_meters is not None:
            for det in detections:
                ...
                distance_roi = self.last_distance_map_meters[y1:y2, x1:x2]
                ...
                if 0 <= cy < self.proc_height and 0 <= cx < self.proc_width and self.cone_mask[cy, cx] == 255:
                    ...
                    hm_cx = int(cx * self.config.get('HEATMAP_RESOLUTION_FACTOR', 0.25))
                    hm_cy = int(cy * self.config.get('HEATMAP_RESOLUTION_FACTOR', 0.25))
    ...
    def process_video(self):
        self._log_message("Iniciando procesamiento de video 3D optimizado...")
...
def main():
    config = {
        ...
        'VIDEO_INPUT_PATH': 'F:/Documents/PycharmProjects/RiskCylinder/GH012372_no_audio.mp4',
    ...
    try:
        ...
        radar.process_video()
        print(f"\n✅ Procesamiento 3D optimizado completado exitosamente!")
